...
We also don't want any of the control samples in our downstream analyses, so we prune them out as well. 


```{r prune-controls}
rem <- sample_names(ps.filt)[grepl('STANDARD|C6',sample_names(ps.filt))] 
ps.filt <- prune_samples(!(sample_names(ps.filt) %in% rem), ps.filt)
