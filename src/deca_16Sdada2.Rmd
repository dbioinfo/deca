...
```
Here we filter the reads with the parameters from above. A summary table describing the strength of this filter is shown. A plot showing the number of reads filtered out of each individual sample is also shown. 
...
library(phangorn)
dna <- refseq(ps)
alignment <- AlignSeqs(dna, anchor = NA) #kwarg 'processors' when null uses all available resources --hefty
phangAlign <- phyDat(as(alignment, "matrix"), type = "DNA") #import to phanghorn
dm     <- dist.ml(phangAlign) #distance matrix
treeNJ <- NJ(dm) #neighbor joining algorithm to get a starting point
fit  <- pml(treeNJ, data = phangAlign)
fitGTR <- optim.pml(fit, model= "GTR") #zero in on the right tree using a biological model of nulceotide substitution
mtree <- fitGTR$tree
phy_tree(ps) <- mtree #attach to phyloseq object
saveRDS(ps, 'data/16Sdada2/phyloseq_dada2.rds')

```

Here we use the package DECIPHER to align the ASVs and the package phanghorn to create a phylogenetic tree from that alignment. First the alignment is computed, this is another expensive operation. Then, a first estimation of the phylogenetic tree is made by neighbor joining (NJ). Then a simple GTR substitution model is used to optimize the tree. The tree is then added to the phyloseq object. 
...
##next a phylogenetic tree 
topten = names(sort(taxa_sums(ps), decreasing = TRUE)[1:10]) #10 most abundant ASV
subset <- prune_taxa(topten, ps)
plot_tree(subset, color='Microhabitat', label.tips='Genus', ladderize='left', justify='left', size='Abundance')

##some exact DNA sequences
head(refseq(ps))

```
First we make an abundance plot. We begin by choosing samples that are specific to Aug 2023 from the SSG microhabitat. There is a somewhat large variance in library size that is evident. This variation is why rarefaction is so important with this dataset. 

Next we create a plot of the top ten most abundant ASVs and show their phylogenetic relationship. Absolute abundance data is shown as dots next to the tree colored by Microhabitat. 

Lastly, we take a look at the exact sequences available in the phyloseq object and confirm they are the right length and show some level of conservation. 