---
title: "deca_16S_graph_func_alpha"
author: "Dylan Barth"
date: "2026-03-31"
...
func <- readRDS('data/16Sfaprotax/faprotax_abund.rds')
```

## Calculate functional alpha indices

Next we calculate each of the metrics we are interested in, these are Richness, Evenness, Shannon and Simpson alpha indices. 

```{r func-alphas}
tmp <- func$res_spe_func_perc

rich <- vegan::specnumber(tmp)
shan <- vegan::diversity(tmp, index = 'shannon')
simp <- vegan::diversity(tmp, index = 'simpson')
peven <- shan / log(rich)
...
#calculate median baseline values for each alpha-microhabitat combo
gdat1 <- gdat %>% filter(SampleDate=='Jul_2022') %>% group_by(Microhabitat, metric) %>% summarize(Alpha=median(Alpha, na.rm = T))
...
g1 <- ggplot(gdat)+
  ...
  geom_hline(data=gdat1,mapping=aes(yintercept=Alpha))+
  ...
  ggtitle("Functional Alpha Diversity")

ggsave('figs/mirl_functional_alpha_boxplot.png', g1, width = 7000, height=4000 ,units='px')
