...
tf <- readRDS('data/cyano/cyano_faprotax_abund.rds')
...
vbles <- colnames(tf$res_spe_func_perc)
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      fluidRow(
        column(width = 6,
          checkboxGroupInput("meta_axes", "Metadata Axis",choices = all_meta_axes, selected = "Microhabitat")
        ),
        column(width = 6,
      ...
      uiOutput("weight_range_ui"),
      
      checkboxInput("use_enrichment", "Weight by FAPROTAX enrichment score", value = TRUE),
      fluidRow(
...
server <- function(input, output, session) {
  ...
  output$weight_range_ui <- renderUI({

    sliderInput("weight_range_log10", "Weight range (log10)",
      min = -5,
      max = 2,
      value = c(1e-3, 1),
      step = 0.05
  ...
  fill_var <- if (length(axes) > 0) axes[[1]] else "variable"
    ...
    #weight filters
    lowpass <- 10^input$weight_range_log10[1]
    highpass <- 10^input$weight_range_log10[2]
    ...
    if (input$use_enrichment){ #(sum of rel abund) x enrichment score from faprotax
      itax <- dat %>% 
        ...
        dplyr::filter(weight >= lowpass, weight <= highpass) %>% 
        pull(tax) %>% unique()
  ...
  mainplot <- eventReactive(input$refresh, { #generate plot on refresh
    ...
    if (is.null(title_txt) || !nzchar(title_txt)) {
      title_txt <- paste("Alluvial:", paste(input$f_groups, collapse = ", "))
    }
  ...
  output$save_plot <- downloadHandler(
    filename = function() {
      nm <- input$plot_title
      if (is.null(nm) || !nzchar(nm)) nm <- "alluvial_plot"
